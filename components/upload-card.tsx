...
export default function UploadCard() {
  ...
  const [emailMessage, setEmailMessage] = useState("Please find the document verification details below:")
  ...
  const handleSendEmail = () => {
    if (!uploadedFile || !recipientEmail) return

    const subject = `Document Verification - Evidence ID: ${uploadedFile.evidenceId}`

    const body =
`${emailMessage}

File Name: ${uploadedFile.file.name}
Evidence ID: ${uploadedFile.evidenceId}
SHA-256 Hash: ${uploadedFile.hash}
${anchorTx ? `Blockchain Transaction: ${anchorTx}` : "Not Anchored Yet"}

Verified via ForeChain ✅`
        ...
        {!uploadedFile ? (
          ...
          <div className="flex flex-col gap-4">
            ...
            <button
              ...
              className="w-full px-4 py-2 rounded-lg bg-primary text-white hover:shadow-md transition-all disabled:opacity-50"
            >
